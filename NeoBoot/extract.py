...
def LanguageUsed():
    ...
    try:
        with open('/etc/enigma2/settings', 'r', encoding='utf-8', errors='ignore') as lang:
...
def getCPUSoC():
    ...
    if os.path.exists('/proc/stb/info/chipset'):
        try:
            with open('/proc/stb/info/chipset', 'r', encoding='utf-8', errors='ignore') as f:
                chipset = f.readline().strip()
            if chipset == '7405(with 3D)':
                # Fixed: Was == (comparison) instead of = (assignment)
                chipset = '7405'
        except IOError:
            pass
    return chipset
...
def getBoxVuModel():
    vumodel = 'UNKNOWN'
    if os.path.exists(
            "/proc/stb/info/vumodel") and not os.path.exists("/proc/stb/info/boxtype"):
        try:
            with open('/proc/stb/info/vumodel', 'r', encoding='utf-8', errors='ignore') as f:
...
def getKernelVersion():
    if not os.path.exists('/tmp/.isnandsim'):
        if os.system(
            'opkg list-installed | grep kernel-module-nandsim' +
                dev_null) != 0:
            os.system('touch /tmp/.isnandsim')

    if os.path.exists('' +
                      getNeoLocation() +
                      'ImagesUpload/dm520') or os.path.exists('' +
                                                              getNeoLocation() +
                                                              'ImagesUpload/dm525'):
        try:
            # Replaced popen with subprocess.check_output
            result = subprocess.check_output(
                ['uname', '-r'], text=True, errors='ignore').strip('\n').split('-')
            kernel_version = result[0]
            return kernel_version
        except Exception:
            os.system('uname -r > /tmp/.uname_r')
            try:
                with open('/tmp/.uname_r', 'r', encoding='utf-8', errors='ignore') as f:
...
def getNeoLocation():
    ...
    if os.path.exists(
        ...
        try:
            with open('/usr/lib/enigma2/python/Plugins/Extensions/NeoBoot/.location', 'r', encoding='utf-8', errors='ignore') as f:
...
def NEOBootMainEx(
    ...
    if not os.path.exists(
        '%s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions' %
            (media, target)):
        os.system('mkdir -p %s/ImageBoot/%s/usr/lib/' % (media, target))
        os.system('mkdir -p %s/ImageBoot/%s/usr/lib/enigma2' % (media, target))
        os.system(
            'mkdir -p %s/ImageBoot/%s/usr/lib/enigma2/python' %
            (media, target))
        os.system(
            'mkdir -p %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins' %
            (media, target))
        os.system(
            'mkdir -p %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions' %
            (media, target))

    if os.path.exists(
        '%s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions/NeoBoot' %
            (media, target)):
        os.system(
            'rm -r %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions/NeoBoot' %
            (media, target))
    ...
    list_two = ['mkdir -p ' + media_target + '/media' + dev_null,
                ...
                'cp -a /usr/share/enigma2/rc_models/* ' + media_target +
                '/usr/share/enigma2/rc_models/' + dev_null,
                'cp -r -p /usr/share/enigma2/rc_models ' +
                media_target + '/usr/share/enigma2' + dev_null,
                'cp -af ' + extensions_path + 'NeoBoot ' +
                media_target + extensions_path + 'NeoBoot' + dev_null,
                'mkdir -p ' + media_target + extensions_path + 'NeoReboot' + dev_null,
                'touch ' + media_target + extensions_path + 'NeoReboot/__init__.py' + dev_null,
                'chmod 644 ' + media_target + extensions_path +
                'NeoReboot/__init__.py' + dev_null,
                'cp -af ' + extensions_path + 'NeoBoot/files/backflash ' +
                media_target + extensions_path + 'NeoReboot/backflash.sh' + dev_null,
    ...
    if CopyFiles == 'False':
        ...
        os.system(
            'touch  ' +
            getNeoLocation() +
            'ImageBoot/.without_copying; sleep 5')

    if CopyKernel == 'True':
        # mips vuplus
        if getBoxHostName() == 'vuultimo' or getCPUSoC() == '7405' and os.path.exists(
                '%s/ImageBoot/%s/etc/vtiversion.info' % (media, target)):
            if os.path.exists(
                '%s/ImageBoot/%s/lib/modules' %
                    (media, target)):
                cmd = 'rm -r %s/ImageBoot/%s/lib/modules' % (media, target)
                rc = os.system(cmd)
            cmd = 'mkdir -p %s/ImageBoot/%s/lib/modules > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
            cmd = 'cp -af /lib/modules  %s/ImageBoot/%s/lib  > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
            if os.path.exists(
                '%s/ImageBoot/%s/lib/firmware' %
                    (media, target)):
                cmd = 'rm -r %s/ImageBoot/%s/lib/firmware' % (
                    media, target)
                rc = os.system(cmd)
            cmd = 'mkdir -p %s/ImageBoot/%s/lib/firmware > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
            cmd = 'cp -af /lib/firmware %s/ImageBoot/%s/lib > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
            os.system(
                'echo "Copied system drivers. Not recommended copied kernel.bin for Ultimo HD."')
            ...
            os.system(
                ...
                dev_null)
            os.system(
                'echo "Copied kernel.bin STB-MIPS Clarke-Tech & Xtrend"')
        # arm vuplus arms
    ...
    if not os.path.exists(
        '' +
        getNeoLocation() +
            'ImageBoot/.without_copying'):
        if os.path.exists('/usr/sbin/nandwrite'):
            cmd = 'cp -af /usr/sbin/nandwrite %s/ImageBoot/%s/usr/sbin/nandwrite > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists('/usr/bin/fullwget'):
            cmd = 'cp -af /usr/bin/fullwget %s/ImageBoot/%s/usr/bin/fullwget > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists('/etc/init.d/inadyn-mt'):
            cmd = 'cp -af /etc/init.d/inadyn-mt %s/ImageBoot/%s/etc/init.d/inadyn-mt > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists('/usr/bin/inadyn-mt'):
            cmd = 'cp -af /usr/bin/inadyn-mt %s/ImageBoot/%s/usr/bin/inadyn-mt > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists('/etc/inadyn.conf'):
            cmd = 'cp -af /etc/inadyn.conf %s/ImageBoot/%s/etc/inadyn.conf > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists(
                '/usr/lib/enigma2/python/Plugins/SystemPlugins/FanControl'):
            cmd = 'cp -af /usr/lib/enigma2/python/Plugins/SystemPlugins/FanControl %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/SystemPlugins > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists('' + extensions_path + 'EmuManager'):
            cmd = 'cp -af ' + extensions_path + \
                'EmuManager %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                    media, target)
            rc = os.system(cmd)
        if os.path.exists('' + extensions_path + 'CamdMenager'):
            cmd = 'cp -af ' + extensions_path + \
                'CamdMenager %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                    media, target)
            rc = os.system(cmd)
        if os.path.exists('' + extensions_path + 'IPTVPlayer'):
            cmd = 'cp -af ' + extensions_path + \
                'IPTVPlayer %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                    media, target)
            rc = os.system(cmd)
            cmd = 'cp /usr/lib/python*.*/htmlentitydefs.pyo %s/ImageBoot/%s/usr/lib/python*.* > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists('' + extensions_path + 'FeedExtra'):
            cmd = 'cp -af ' + extensions_path + \
                'FeedExtra %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                    media, target)
            rc = os.system(cmd)
        if os.path.exists('' + extensions_path + 'MyUpdater'):
            cmd = 'cp -af ' + extensions_path + \
                'MyUpdater %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                    media, target)
            rc = os.system(cmd)
        ...
        if os.path.exists('' + extensions_path + 'TempFanControl'):
            cmd = 'cp -af ' + extensions_path + \
                'TempFanControl %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/TempFanControl > /dev/null 2>&1' % (
                    media, target)
            rc = os.system(cmd)
        if not os.path.exists(
            '%s/ImageBoot/%s/usr/lib/enigma2/python/boxbranding.so' %
                (media, target)):
            cmd = 'cp -af /usr/lib/enigma2/python/boxbranding.so %s/ImageBoot/%s/usr/lib/enigma2/python/boxbranding.so > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        if os.path.exists(
            '%s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions/HbbTV' %
                (media, target)):
            os.system(
                'rm -rf %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions/HbbTV' %
                (media, target))

        if TvList == 'True':
            if not os.path.exists(
                '%s/ImageBoot/%s/etc/enigma2' %
                    (media, target)):
                cmd = 'mkdir -p %s/ImageBoot/%s/etc/enigma2' % (media, target)
                rc = os.system(cmd)
            cmd = 'cp /etc/enigma2/*.tv %s/ImageBoot/%s/etc/enigma2' % (
                media, target)
            rc = os.system(cmd)
            cmd = 'cp /etc/enigma2/*.radio %s/ImageBoot/%s/etc/enigma2' % (
                media, target)
            rc = os.system(cmd)
            cmd = 'cp /etc/enigma2/*.tv %s/ImageBoot/%s/etc/enigma2' % (
                media, target)
            rc = os.system(cmd)
            cmd = 'cp /etc/enigma2/lamedb %s/ImageBoot/%s/etc/enigma2' % (
                media, target)
            rc = os.system(cmd)
        ...
        if LanWlan == 'True':
            if os.path.exists(
                '%s/ImageBoot/%s/etc/vtiversion.info' %
                    (media, target)):
                os.system(
                    'echo "Not copied LAN-WLAN, not recommended for this image."')
            ...
            if os.path.exists('/etc/wpa_supplicant.wlan0.conf'):
                cmd = 'cp -af /etc/wpa_supplicant.wlan0.conf %s/ImageBoot/%s/etc/wpa_supplicant.wlan0.conf > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
            if os.path.exists('/etc/network/interfaces'):
                cmd = 'cp -af /etc/network/interfaces %s/ImageBoot/%s/etc/network/interfaces > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
            if os.path.exists('/etc/wpa_supplicant.conf'):
                cmd = 'cp -af /etc/wpa_suppligetinstallscant.conf %s/ImageBoot/%s/etc/wpa_supplicant.conf > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
            if os.path.exists('/etc/resolv.conf'):
                cmd = 'cp -af /etc/resolv.conf %s/ImageBoot/%s/etc/resolv.conf > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
            if os.path.exists('/etc/wl.conf.wlan3'):
                cmd = 'cp -af /etc/wl.conf.wlan3 %s/ImageBoot/%s/etc/wl.conf.wlan3 > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
        ...
        if Montowanie == 'True':
            if getCPUtype() == "MIPS":
                if os.path.exists(
                    '%s/ImageBoot/%s/etc/fstab' %
                        (media, target)):
                    cmd = 'mv %s/ImageBoot/%s/etc/fstab %s/ImageBoot/%s/etc/fstab.org' % (
                        media, target, media, target)
                    rc = os.system(cmd)
                if os.path.exists(
                    '%s/ImageBoot/%s/etc/init.d/volatile-media.sh' %
                        (media, target)):
                    cmd = 'mv %s/ImageBoot/%s/etc/init.d/volatile-media.sh %s/ImageBoot/%s/etc/init.d/volatile-media.sh.org' % (
                        media, target, media, target)
                    rc = os.system(cmd)
                cmd = 'cp -r /etc/fstab %s/ImageBoot/%s/etc/fstab' % (
                    media, target)
                rc = os.system(cmd)
        ...
        if InstallSettings == 'True':
            if not os.path.exists(
                ...
                cmd = 'mkdir -p %s/ImageBoot/%s/etc/enigma2' % (media, target)
                rc = os.system(cmd)
            cmd = 'cp /etc/enigma2/settings %s/ImageBoot/%s/etc/enigma2' % (
                media, target)
            rc = os.system(cmd)
            if not os.path.exists(
                '%s/ImageBoot/%s/etc/tuxbox/config' %
                    (media, target)):
                cmd = 'mkdir -p /etc/tuxbox/config %s/ImageBoot/%s/etc/tuxbox/config' % (
                    media, target)
                rc = os.system(cmd)
                cmd = 'mkdir -p /etc/tuxbox/scce %s/ImageBoot/%s/etc/tuxbox/scce' % (
                    media, target)
                rc = os.system(cmd)
            cmd = 'cp -af /etc/tuxbox/* %s/ImageBoot/%s/etc/tuxbox' % (
                media, target)
            rc = os.system(cmd)
            os.system(
                'touch /tmp/settings_copied; echo "System settings copied..."')
        ...
        if RepairFTP == 'True':
            ...
            if os.path.exists(filename):
                ...
                try:
                    ...
                    with open(filename2, 'w', encoding='utf-8', errors='ignore') as out:
                        for line in lines:
                    ...
                    os.system(f'echo "Failed to patch {filename}: {e}"')
        ...
        if SoftCam == 'True':
            if os.path.exists('/etc/CCcam.cfg'):
                cmd = 'cp -af /etc/CCcam.cfg %s/ImageBoot/%s/etc > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
            if os.path.exists('/etc/tuxbox/config'):
                cmd = 'cp -af /etc/tuxbox/config %s/ImageBoot/%s/etc/tuxbox > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
        ...
        if MediaPortal == 'True':
            if os.path.exists('' + extensions_path + 'MediaPortal'):
                cmd = 'cp -af ' + extensions_path + \
                    'MediaPortal %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                        media, target)
                rc = os.system(cmd)
                cmd = 'cp -af ' + extensions_path + \
                    'mpgz %s/ImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions > /dev/null 2>&1' % (
                        media, target)
                rc = os.system(cmd)
                cmd = 'cp -af /usr/lib/python2.7/argparse.pyo %s/ImageBoot/%s/usr/lib/python2.7 > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
                cmd = 'cp -af /usr/lib/python2.7/robotparser.pyo %s/ImageBoot/%s/usr/lib/python2.7 > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
                cmd = 'cp -af /usr/lib/python2.7/site-packages/Crypto %s/ImageBoot/%s/usr/lib/python2.7/site-packages > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
                cmd = 'cp -af /usr/lib/python2.7/site-packages/mechanize %s/ImageBoot/%s/usr/lib/python2.7/site-packages > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
                cmd = 'cp -af /usr/lib/python2.7/site-packages/requests %s/ImageBoot/%s/usr/lib/python2.7/site-packages > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
                cmd = 'cp -af /usr/lib/python2.7/site-packages/requests-2.11.1-py2.7.egg-info %s/ImageBoot/%s/usr/lib/python2.7/site-packages > /dev/null 2>&1' % (
                    media, target)
                rc = os.system(cmd)
                ...
                if not os.path.exists(
                    '%s/ImageBoot/%s/etc/enigma2' %
                        (media, target)):
                    cmd = 'mkdir -p %s/ImageBoot/%s/etc/enigma2' % (
                        media, target)
                    rc = os.system(cmd)
                if os.path.exists('/etc/enigma2/mp_2s4p'):
                    cmd = 'cp /etc/enigma2/mp_2s4p %s/ImageBoot/%s/etc/enigma2' % (
                        media, target)
                    rc = os.system(cmd)
                if os.path.exists('/etc/enigma2/mp_config'):
                    cmd = 'cp /etc/enigma2/mp_config %s/ImageBoot/%s/etc/enigma2' % (
                        media, target)
                    rc = os.system(cmd)
                if os.path.exists('/etc/enigma2/mp_pluginliste'):
                    cmd = 'cp /etc/enigma2/mp_pluginliste %s/ImageBoot/%s/etc/enigma2' % (
                        media, target)
                    rc = os.system(cmd)
        ...
        if PiconR == 'True':
            if os.path.exists('/usr/share/enigma2/picon'):
                cmd = 'cp -af /usr/share/enigma2/picon %s/ImageBoot/%s/usr/share/enigma2' % (
                    media, target)
                rc = os.system(cmd)
        ...
        if Kodi == 'True':
            cmd = 'mkdir -p %s/ImageBoot/%s/home/root/.kodi > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
        ...
        if BlackHole == 'True':
            if 'BlackHole' in source or os.path.exists(
                '%s/ImageBoot/%s/usr/lib/enigma2/python/Blackhole' %
                    (media, target)):
                cmd = 'mkdir -p ' + getNeoLocation() + 'ImageBoot/%s/boot/blackhole' % target
                rc = os.system(cmd)
                cmd = 'mv ' + getNeoLocation() + 'ImageBoot/' + target + '/usr/lib/enigma2/python/Blackhole/BhUtils.pyo ' + \
                    getNeoLocation() + 'ImageBoot/%s/usr/lib/enigma2/python/Blackhole/BhUtils.pyo.org' % target
                rc = os.system(cmd)
                cmd = 'cp -af ' + extensions_path + 'NeoBoot/bin/utilsbh ' + \
                    getNeoLocation() + 'ImageBoot/%s/usr/lib/enigma2/python/Blackhole/BhUtils.py' % target
                rc = os.system(cmd)
                ...
                try:
                    with open(localfile, 'w', encoding='utf-8', errors='ignore') as temp_file:
                        temp_file.write(text)
                except Exception as e:
                    os.system(f'echo "Failed to write {localfile}: {e}"')

                cmd = 'mv ' + getNeoLocation() + 'ImageBoot/' + target + '/usr/bin/enigma2 ' + \
                    getNeoLocation() + 'ImageBoot/%s/usr/bin/enigma2-or' % target
                rc = os.system(cmd)
                ...
                try:
                    ...
                    os.system(f'echo "Failed to read {fail}: {e}"')

                if content:
                    localfile2 = '' + getNeoLocation() + 'ImageBoot/%s/usr/bin/enigma2' % target
                    try:
                        with open(localfile2, 'w', encoding='utf-8', errors='ignore') as temp_file2:
                            temp_file2.write(
                                content.replace(
                                    '/proc/blackhole/version',
                                    '/boot/blackhole/version'))
...
# for all image:
        ...
        if os.path.exists('/var/lib/tailscale/tailscaled.state'):
            ...
            if not os.path.exists(
                ...
                if not os.path.exists(
                    '%s/ImageBoot/%s/lib/modules/kernel/drivers/net' %
                        (media, target)):
                    # cmd = 'mkdir -p ' + getNeoLocation() +
                    # 'ImageBoot/%s/lib/modules/' + getKernelVersion()
                    # + '/kernel/drivers/net/' % target
                    cmd = 'mkdir -p %s/ImageBoot/%s/lib/modules/kernel/drivers/net/' % (
                        media, target)
                    rc = os.system(cmd)
                    cmd = 'cp -af /lib/modules/' + getKernelVersion() + \
                        '/kernel/drivers/net/tun.ko %s/ImageBoot/%s/lib/modules/kernel/drivers/net/' % (
                        media, target)
                    rc = os.system(cmd)
            if not os.path.exists(
                '%s/ImageBoot/%s/var/run/tailscale' %
                    (media, target)):
            ...
            if os.path.exists('/run/tailscale'):
                ...
                rc = os.system(cmd)
                cmd1 = 'cp -aRf /run/resolvconf/interfaces/tailscale ' + \
                    getNeoLocation() + 'ImageBoot/%s/run/tailscale/' % target
                rc = os.system(cmd1)

        if os.path.exists('%s/ImageBoot/%s/etc/init.d' % (media, target)):
            cmd = 'ln -s %sImageBoot/%s/usr/lib/enigma2/python/Plugins/Extensions/NeoBoot/files/userscript.sh %sImageBoot/%s/etc/rcS.d/S99neo.local' % (
                media, target, media, target)
    ...
    if not os.path.exists(
        '' +
        getNeoLocation() +
            'ImageBoot/.without_copying') and not os.path.exists(' /tmp/settings_copied'):
        try:
            with open("/etc/hostname", 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    if getCPUtype() == 'MIPS' and "dm500hd" in line or "dm800se" in line or "dm800" in line or "dm800se" in line or "dm8000" in line:
                        if os.path.exists(
                            '%s/ImageBoot/%s/etc/enigma2' %
                                (media, target)):
                            cmd = 'rm -r %s/ImageBoot/%s/etc/enigma2' % (
                                media, target)
                            rc = os.system(cmd)
                    else:
                        if not os.path.exists(
                            ...
                            cmd = 'mkdir -p %s/ImageBoot/%s/etc/enigma2' % (
                                media, target)
                            rc = os.system(cmd)
                            cmd = 'touch %s/ImageBoot/%s/etc/enigma2/settings' % (
                                media, target)
                            rc = os.system(cmd)
                        cmd = 'grep "config.Nims" /etc/enigma2/settings >> %s/ImageBoot/%s/etc/enigma2/settings' % (
                            media, target)
                        rc = os.system(cmd)
                        cmd1 = 'grep "av.videomode.DVI" /etc/enigma2/settings >> %s/ImageBoot/%s/etc/enigma2/settings' % (
                            media, target)
                        rc = os.system(cmd1)
                        cmd2 = 'grep "config.OpenWebif" /etc/enigma2/settings >> %s/ImageBoot/%s/etc/enigma2/settings' % (
                            media, target)
                        rc = os.system(cmd2)
                        cmd3 = 'grep "config.osd" /etc/enigma2/settings >> %s/ImageBoot/%s/etc/enigma2/settings' % (
                            media, target)
                        rc = os.system(cmd3)
                        cmd4 = 'grep "config.timezone.val" /etc/enigma2/settings >> %s/ImageBoot/%s/etc/enigma2/settings' % (
                            media, target)
                        rc = os.system(cmd4)
                        cmd5 = 'grep "config.servicelist.startuproot" /etc/enigma2/settings >> %s/ImageBoot/%s/etc/enigma2/settings' % (
                            media, target)
                        rc = os.system(cmd5)
                        cmd6 = 'grep "UUID=" /etc/fstab >> %s/ImageBoot/%s/etc/fstab' % (
                            media, target)
                        rc = os.system(cmd6)
        except Exception as e:
            os.system(f'echo "Failed reading /etc/hostname: {e}"')
...
#####################################
        if not os.path.exists(
            ...
            if os.path.exists(namefile):  # Check if source exists
                try:
                    with open(namefile, 'r', encoding='utf-8', errors='ignore') as f:
                        lines = f.readlines()
                    with open(namefile2, 'w', encoding='utf-8', errors='ignore') as out:
                        for line in lines:
                    ...
                    os.system(f'echo "Failed to patch {namefile}: {e}"')
            ...
            if os.path.exists(fname):
                ...
                try:
                    with open(fname, 'r', encoding='utf-8', errors='ignore') as f:
                        lines = f.readlines()
                    with open(fname2, 'w', encoding='utf-8', errors='ignore') as out:
                        for line in lines:
                            if line.find(
                                    'if file(""/proc/stb/info/vumodel")') != -1:
                                line = '#' + line
                            out.write(line)
                    os.rename(fname2, fname)
                except Exception as e:
                    os.system(f'echo "Failed to patch {fname}: {e}"')
            ...
            if os.path.exists(targetfile):
                ...
                try:
                    with open(targetfile, 'r', encoding='utf-8', errors='ignore') as f:
                        lines = f.readlines()
                    with open(targetfile2, 'w', encoding='utf-8', errors='ignore') as out:
                        for line in lines:
                            if not line.startswith('nopriv_user'):
                                out.write(line)
                    os.rename(targetfile2, targetfile)
                except Exception as e:
                    os.system(f'echo "Failed to patch {targetfile}: {e}"')
                    if os.path.exists(targetfile2):
                        os.remove(targetfile2)

            mypath = media + '/ImageBoot/' + target + '/usr/lib/opkg/info/'
            cmd = 'mkdir -p %s/ImageBoot/%s/var/lib/opkg/info > /dev/null 2>&1' % (
                media, target)
            rc = os.system(cmd)
            if not os.path.exists(mypath):
                mypath = media + '/ImageBoot/' + target + '/var/lib/opkg/info/'

            if os.path.exists(mypath):  # Check if path exists before listing
                for fn in os.listdir(mypath):
                    if (fn.find('kernel-image') != -1 and fn.find('postinst') != -1) or \
                       fn.find('-bootlogo.postinst') != -1 or \
                       fn.find('-bootlogo.postrm') != -1 or \
                       fn.find('-bootlogo.preinst') != -1 or \
                       fn.find('-bootlogo.prerm') != -1:

                        filename = os.path.join(mypath, fn)
                        filename2 = filename + '.tmp'
                        try:
                            with open(filename, 'r', encoding='utf-8', errors='ignore') as f:
                                lines = f.readlines()
                            ...
                            os.system(
                                f'echo "Failed to patch {filename}: {e}"')
                            if os.path.exists(filename2):
                                os.remove(filename2)

            # _______________________________________________status1 zmienione
            if os.path.exists(
                '%s/ImageBoot/%s/var/lib/opkg/status1' %
                    (media, target)):
                cmd = 'mv ' + getNeoLocation() + 'ImageBoot/' + target + '/var/lib/opkg/status ' + \
                    getNeoLocation() + 'ImageBoot/%s/var/lib/opkg/status-or' % target
                rc = os.system(cmd)
...
#    rc = os.system(cmd)
    if os.path.exists('%s/ImageBoot/%s/usr/lib' % (media, target)):
        cmd = 'cp -af /usr/lib/periodon ' + \
            getNeoLocation() + 'ImageBoot/%s/usr/lib/' % target
        rc = os.system(cmd)
        cmd = 'mkdir -p ' + getNeoLocation() + \
            'ImageBoot/%s/usr/lib/enigma2/python/Tools/' % target
        rc = os.system(cmd)
        cmd = 'cp -af /usr/lib/enigma2/python/Tools/Testinout.py ' + \
            getNeoLocation() + 'ImageBoot/%s/usr/lib/enigma2/python/Tools/' % target
        rc = os.system(cmd)
    ...
    os.system(
        'touch ' +
        getNeoLocation() +
        'ImageBoot/.data; echo "Data instalacji image" > ' +
        getNeoLocation() +
        'ImageBoot/.data; echo " "; date  > ' +
        getNeoLocation() +
        'ImageBoot/.data')
    os.system(
        'mv -f ' +
    ...
    try:
        with open(mediahome + '.neonextboot', 'w', encoding='utf-8', errors='ignore') as out:
            out.write(target)
    except Exception as e:
        os.system(f'echo "Failed to write .neonextboot: {e}"')
    ...
    if '.tar.xz' not in source and not os.path.exists(
            '' + getNeoLocation() + '/ImageBoot/%s/etc/issue' % target):
        ...
        os.system(
            'echo "The installed system may not start. Check the correctness of the installed image directory!!!"')
    ...
    os.system(
        'echo "If you want to save the installation process from the console press green."')
...
def RemoveUnpackDirs():
    ...
    if os.path.exists('' + getNeoLocation() + 'ImagesUpload/unpackedzip'):
        ...
        rc = os.system(
            'rm -r ' +
            getNeoLocation() +
            'ImagesUpload/unforce_multibox.txt')
        ...
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/usb_update.bin ' +
            getNeoLocation() +
            'ImagesUpload/octagon; rm -r ' +
            getNeoLocation() +
        ...
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/bootargs.bin ' +
            getNeoLocation() +
            'ImagesUpload/h5; mv ' +
            getNeoLocation() +
            'ImagesUpload/fastboot.bin ' +
            getNeoLocation() +
            'ImagesUpload/h5')
        ...
        rc = os.system(
            ...
            'ImagesUpload/bootargs.bin ' +
            getNeoLocation() +
            'ImagesUpload/i55plus; mv ' +
            getNeoLocation() +
            'ImagesUpload/fastboot.bin ' +
            getNeoLocation() +
            'ImagesUpload/i55plus')
        rc = os.system('rm -r ' + getNeoLocation() + 'ImagesUpload/i55plus')
    elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/h9combo'):
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/force_h9combo_READ.ME ' +
            getNeoLocation() +
            'ImagesUpload/h9combo; mv ' +
            getNeoLocation() +
            'ImagesUpload/unforce_h9combo.txt ' +
            getNeoLocation() +
            'ImagesUpload/h9combo')
        rc = os.system('rm -r ' + getNeoLocation() + 'ImagesUpload/h9combo')
    elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/h9combose'):
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/force_h9combose_READ.ME ' +
            getNeoLocation() +
            'ImagesUpload/h9combo; mv ' +
            getNeoLocation() +
            'ImagesUpload/unforce_h9combose.txt ' +
            getNeoLocation() +
        ...
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/force_h10_READ.ME ' +
            getNeoLocation() +
            'ImagesUpload/h10; mv ' +
            getNeoLocation() +
            'ImagesUpload/unforce_h10.txt ' +
            getNeoLocation() +
            'ImagesUpload/h10')
        ...
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/force_h11_READ.ME ' +
            getNeoLocation() +
            'ImagesUpload/h11; mv ' +
            getNeoLocation() +
            'ImagesUpload/unforce_h11.txt ' +
            getNeoLocation() +
            'ImagesUpload/h11')
        ...
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/apploader.bin ' +
            getNeoLocation() +
            'ImagesUpload/beyonwiz')
        rc = os.system(
            'mv ' +
            getNeoLocation() +
            'ImagesUpload/bootargs.bin ' +
        ...
        if os.path.exists(
            '' +
            getNeoLocation() +
                'ImagesUpload/pulse4k/force_pulse4k_READ.ME'):
            rc = os.system(
                'rm -r ' +
                getNeoLocation() +
                'ImagesUpload/pulse4k/force_pulse4k_READ.ME; rm -r ' +
                getNeoLocation() +
                'ImagesUpload/pulse4k/unforce_pulse4k.txt')
        rc = os.system('rm -r ' + getNeoLocation() + 'ImagesUpload/pulse4k')
    elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/pulse4kmin'):
        if os.path.exists(
            '' +
            getNeoLocation() +
                'ImagesUpload/pulse4kmin/force_pulse4kmini_READ.ME'):
            rc = os.system(
                'rm -r ' +
                getNeoLocation() +
                'ImagesUpload/pulse4kmini/force_pulse4kmini_READ.ME; rm -r ' +
                getNeoLocation() +
                'ImagesUpload/pulse4kmini/unforce_pulse4kmini.txt')
        rc = os.system(
            'rm -r ' +
            getNeoLocation() +
            'ImagesUpload/pulse4kmini')
    ...
    if os.path.exists(
        '' +
        getNeoLocation() +
            'ImagesUpload/update_bootargs_h8'):
        rc = os.system(
            'rm -r ' +
            getNeoLocation() +
            'ImagesUpload/update_bootargs_h8')
    if os.path.exists(
        '' +
        getNeoLocation() +
            'ImagesUpload/force_hd60_READ.ME'):
        rc = os.system(
            'rm -r ' +
            getNeoLocation() +
            'ImagesUpload/force_hd60_READ.ME')
...
def NEOBootExtract(source, target, ZipDelete, Nandsim):
    ...
    if os.path.exists(sourcefile2):
        if sourcefile2.endswith('.nfi'):
            ...
            rc = os.system(cmd)
            cmd = 'touch /tmp/root_jffs2; ' + extensions_path + 'NeoBoot/bin/nfidump ' + \
                sourcefile2 + ' ' + getNeoLocation() + 'ImageBoot/' + target
            rc = os.system(cmd)
            if os.path.exists(
                    '%sImageBoot/%s/media/squashfs-images' %
                    (media, target)) and os.path.exists(
                    '%s/squashfs-images' %
                    (media)):
                os.system(
                    'cp -af %s/squashfs-images/* "%sImageBoot/%s/media/squashfs-images' %
                    (media, media, target))
            if ZipDelete == 'True':
                rc = os.system('rm -rf ' + sourcefile2)
            else:
                os.system(
                    'echo "NeoBoot keep the file:  %s  for reinstallation."' %
                    sourcefile2)
    # Instalacja *.rar
    if os.path.exists(sourcefile3):
        if sourcefile3.endswith('.rar'):
            os.system('echo "Installing iamge  x.rar..."')
            cmd = 'unrar e ' + sourcefile3 + ' ' + \
                getNeoLocation() + 'ImagesUpload/ > /dev/null 2>&1'
            rc = os.system(cmd)
            if ZipDelete == 'True':
    ...
    fn = 'NewImage'
    sourcelist = []
    if os.path.exists('%sImagesUpload' % getNeoLocation()):
    ...
    # Instalacja MIPS
    if getCPUtype() == 'MIPS' and not os.path.exists(
            '/tmp/root_jffs2') and not os.path.exists('/tmp/other_image'):
        if not os.path.exists('' + getNeoLocation() + 'ubi'):
        ...
        # NANDSIM
        if Nandsim == 'True' or os.path.exists('/tmp/.isnandsim') or os.path.exists(
                '/lib/modules/%s/kernel/drivers/mtd/nand/nandsim.ko' % getKernelVersion()):
            for i in range(0, 20):
            ...
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/zgemma'):
                ...
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/zgemma/sh1'):
                    os.chdir('sh1')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/zgemma/sh2'):
                    os.chdir('sh2')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/zgemma/h2'):
                    os.chdir('h2')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/zgemma/h3'):
                    os.chdir('h3')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/zgemma/h5'):
                    os.chdir('h5')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/zgemma/h7'):
                    os.chdir('h7')
            ...
            if os.path.exists(
                ...
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/mini'):
                    os.chdir('mini')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/miniplus'):
                    os.chdir('miniplus')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/minihybrid'):
                    os.chdir('minihybrid')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/twin'):
                    os.chdir('twin')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/ultra'):
                    os.chdir('ultra')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/micro'):
                    os.chdir('micro')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/twinplus'):
                    os.chdir('twinplus')
            # atemio
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/atemio'):
                os.chdir('atemio')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/atemio/5x00'):
                    os.chdir('5x00')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/atemio/6000'):
                    os.chdir('6000')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/atemio/6100'):
                    os.chdir('6100')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/atemio/6200'):
                    os.chdir('6200')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/atemio/8x00'):
                    os.chdir('8x00')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/atemio/8x00'):
                    os.chdir('8x00')
            ...
            if os.path.exists(
                '' +
                getNeoLocation() +
                    'ImagesUpload/formuler1'):
                os.chdir('formuler1')
                brand = 'formuler1'
            if os.path.exists(
                '' +
                getNeoLocation() +
                    'ImagesUpload/formuler2'):
                os.chdir('formuler2')
                brand = 'formuler2'
            if os.path.exists(
                '' +
                getNeoLocation() +
                    'ImagesUpload/formuler3'):
                os.chdir('formuler3')
                brand = 'formuler3'
            if os.path.exists(
                '' +
            ...
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/gigablue'):
                ...
                rootfname = 'rootfs.bin'
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/gigablue/x1'):
                    os.chdir('x1')
            # VuPlus
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/vuplus'):
                ...
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/uno'):
                    os.chdir('uno')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/duo'):
                    os.chdir('duo')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/vuplus/ultimo'):
                    os.chdir('ultimo')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/solo'):
                    os.chdir('solo')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/duo2'):
                    os.chdir('duo2')
                    rootfname = 'root_cfe_auto.bin'
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/solo2'):
                    os.chdir('solo2')
                    rootfname = 'root_cfe_auto.bin'
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/vuplus/solose'):
                    os.chdir('solose')
                    rootfname = 'root_cfe_auto.bin'
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/zero'):
                    os.chdir('zero')
                    rootfname = 'root_cfe_auto.bin'
            ...
            if os.path.exists(
                '' +
                getNeoLocation() +
                    'ImagesUpload/dm520') and not os.path.exists('/tmp/dm_image'):
            ...
            os.system(
                'echo "If the image installation failed, try disabling the use of nandsim installation"')
            # rc = os.system('insmod /lib/modules/' + getKernelVersion() +
            # '/kernel/drivers/mtd/nand/nandsim.ko cache_file=' +
            # getNeoLocation() + 'image_cache first_id_byte=0x20
            # second_id_byte=0xaa third_id_byte=0x00 fourth_id_byte=0x15;sleep
            # 5')#% getKernelVersion())
            if not os.path.exists(
                '/lib/modules/%s/kernel/drivers/mtd/nand/nandsim.ko' %
                    getKernelVersion()):
                rc = os.system(
                    'insmod /tmp/nandsim.ko cache_file=' +
                    getNeoLocation() +
                    'image_cache first_id_byte=0x20 second_id_byte=0xaa third_id_byte=0x00 fourth_id_byte=0x15;sleep 5')  # % getKernelVersion())
            else:
                rc = os.system(
                    'insmod /lib/modules/' +
                    getKernelVersion() +
            ...
            os.chdir('/home/root')
            cmd = 'cp -af ' + getNeoLocation() + 'ubi/* ' + getNeoLocation() + \
                'ImageBoot/' + target
            rc = os.system(cmd)
            ...
            if os.path.exists(
                    '/tmp/dm_image') and os.path.exists('%s/ImageBoot/%s/etc/issue' % (media, target)):
                os.system('rm -f /tmp/dm_image')

            if '.tar.xz' not in source and not os.path.exists(
                    '%s/ImageBoot/%s/etc/issue' % (media, target)):
                RemoveUnpackDirs()
                os.system("echo 3 > /proc/sys/vm/drop_caches")
                os.system(
                    'echo ""; echo "Nie zainstalowano systemu ! Powodem b\xc5\x82\xc4\x99du instalacji mo\xc5\xbce by\xc4\x87 kernel-module-nandsim."')
                os.system(
                    'echo "By uzyc innego narzedzia do rozpakowania image, ponow instalacje image jeszcze raz po restarcie tunera."')
        ...
        # UBI_READER
            ...
            if os.path.exists(
                ...
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/microv2'):
                    os.chdir('microv2')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                ...
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/mini4k'):
                    os.chdir('mini4k')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/miraclebox/ultra4k'):
                    os.chdir('ultra4k')
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/atemio'):
                os.chdir('atemio')
                if os.path.exists(
            ...
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/beyonwiz'):
                os.chdir('beyonwiz')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/beyonwiz/hdx'):
                    os.chdir('hdx')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/beyonwiz/hdp'):
                    os.chdir('hdp')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/beyonwiz/hde2'):
                    os.chdir('hde2')
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/vuplus'):
                os.chdir('vuplus')
                if os.path.exists(
                ...
                if os.path.exists(
                    ...
                    if os.path.exists(
                        '' +
                        getNeoLocation() +
                            'ImagesUpload/vuplus/solose/root_cfe_auto.bin'):
                        os.chdir('solose')
                ...
                if os.path.exists(
                    ...
                    os.chdir('solo4k')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/uno4k'):
                    os.chdir('uno4k')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/vuplus/ultimo4k'):
                    os.chdir('ultimo4k')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/vuplus/duo4k'):
                    os.chdir('duo4k')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/vuplus/duo4kse'):
                    os.chdir('duo4kse')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/vuplus/zero4k'):
                    os.chdir('zero4k')
                if os.path.exists(
                    '' +
                    getNeoLocation() +
                        'ImagesUpload/vuplus/uno4kse'):
                    os.chdir('uno4kse')
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/et10000'):
                os.chdir('et10000')
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/et9x00'):
            ...
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/patch.e2'):
                os.system(
                    'rm -f ' +
                    getNeoLocation() +
                    'ImagesUpload/patch.e2 ')
                os.system('echo "____NEOBOOT will not unpack this image.____"')
                os.system(
                    'echo "____Try to install the image vuultimo mips____"')
            # Vimastec
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/vs1000'):
                os.chdir('vs1000')
            # Dreambox
            ...
            if os.path.exists(
                '' +
                getNeoLocation() +
                    'ImagesUpload/vuplus/root_cfe_auto.*'):
                os.system('mv -f root_cfe_auto.* rootfs.bin')
            cmd = 'chmod 777 ' + extensions_path + 'NeoBoot/ubi_reader/ubi_extract_files.py'
            rc = os.system(cmd)
            cmd = 'python ' + extensions_path + \
                'NeoBoot/ubi_reader/ubi_extract_files.py rootfs.bin -o' + getNeoLocation() + 'ubi'
            rc = os.system(cmd)
            os.chdir('/home/root')
            ...
            os.system(
                'echo "NeoBoot wykrył błąd !!! Prawdopodobnie brak ubi_reader lub nandsim."')
...
# ARM
        ...
        if os.path.exists('' +
                          getNeoLocation() +
                          'ImagesUpload/h9/rootfs.ubi') or os.path.exists('' +
                                                                          getNeoLocation() +
                                                                          'ImagesUpload/h8/rootfs.ubi') or os.path.exists('' +
                                                                                                                          getNeoLocation() +
                                                                                                                          'ImagesUpload/e2/update/rootfs.ubi'):
            if os.path.exists(
                '' +
                getNeoLocation() +
                    'ImagesUpload/h9/rootfs.ubi'):
                os.chdir('h9')
            ...
            os.system('echo "Instalacja - ubi_reader w toku..."')
            print(
                "[NeoBoot] Extracting UBIFS image and moving extracted image to our target")
            cmd = 'chmod 777 ' + extensions_path + 'NeoBoot/ubi_reader/ubi_extract_files.py'
            rc = os.system(cmd)
            cmd = 'python ' + extensions_path + \
                'NeoBoot/ubi_reader/ubi_extract_files.py rootfs.bin -o ' + getNeoLocation() + 'ubi'
            rc = os.system(cmd)
            os.chdir('/home/root')
            cmd = 'cp -af -p ' + getNeoLocation() + 'ubi/rootfs/* ' + \
                getNeoLocation() + 'ImageBoot/' + target
            rc = os.system(cmd)
            cmd = 'chmod -R +x ' + getNeoLocation() + 'ImageBoot/' + target
            rc = os.system(cmd)
            ...
            rc = os.system(cmd)
            os.chdir('/home/root')
            cmd = 'cp -af ' + getNeoLocation() + 'ubi/rootfs/* ' + \
                getNeoLocation() + 'ImageBoot/' + target
            rc = os.system(cmd)
            cmd = 'chmod -R +x ' + getNeoLocation() + 'ImageBoot/' + target
            rc = os.system(cmd)
            ...
            os.system(
                'mv -f ' +
                getNeoLocation() +
                'ImagesUpload/update/lunix/cfe/oe_rootfs.bin ' +
                getNeoLocation() +
                'ImagesUpload/update/rootfs.bin')
            os.system('echo "Instalacja - ubi_reader w toku..."')
            print(
                "[NeoBoot] Extracting UBIFS image and moving extracted image to our target")
            cmd = 'chmod 777 ' + extensions_path + 'NeoBoot/ubi_reader/ubi_extract_files.py'
        ...
        # vuplus________________________
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/solo4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/solo4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/vuplus/uno4k'):
            os.system(
                'echo "Please wait. System installation dla modelu VuPlus Uno4K."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/uno4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/uno4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/uno4kse/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/uno4kse/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/zero4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/zero4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/ultimo4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/ultimo4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/vuplus/duo4k'):
            os.system('echo "Please wait. System installation VuPlus Duo4k."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/duo4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/duo4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vuplus/duo4kse/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vuplus/duo4kse/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        # ________________________________
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/sf4008'):
            os.system('echo "Please wait. System installation Octagon SF4008."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/sf4008/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/octagon/sf8008m/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/octagon/sf8008m/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/octagon/sf8008'):
            os.system('echo "Please wait. System installation Octagon SF8008."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/octagon/sf8008/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/octagon/sf8008/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/osmio4k'):
            os.system('echo "Please wait. System installation EDISION osmio4k"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/osmio4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/osmio4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/osmio4kplus'):
            os.system(
                'echo "Please wait. System installation EDISION osmio4kplus"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/osmio4kplus/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/osmio4kplus/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/osmini4k'):
            os.system(
                'echo "Please wait. System installation Edision OS mini 4K"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/osmini4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/osmini4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/dm900'):
            ...
            os.system(
                'echo "Please wait. System installation Dreambox dreamtwo."')
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/hd60/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/hd60/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/hd61'):
            os.system('echo "Please wait. System installation AX HD60 4K"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/hd61/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/hd61/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
...
#            rc = os.system(cmd)
            ...
            os.system(
                'echo "Please wait. System installation AX multi twin or combo"')
            ...
            os.system('echo "Please wait. System installation novaler 4k pro"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/multiboxpro/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/multiboxpro/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/axas/axasc4k'):
            os.system('echo "Please wait. System installation Axas his c4k"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/axas/axasc4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/axas/axasc4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/e4hd'):
            os.system(
                'echo "Please wait. System installation Axas E4HD 4K Ultra w toku..."')
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/gigablue/ue4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/gigablue/ue4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/update/revo4k'):
            os.system('echo "Please wait. System installation Revo4k."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/update/revo4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/update/revo4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            os.system(
                'echo "Please wait. System installation Zgemma h9combose ."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/h9combose/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/h9combose/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/h11/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/h11/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/miraclebox/mini4k'):
            os.system(
                'echo "Please wait. System installation Miraclebox mini4k."')
            ...
            os.system(
                'echo "Please wait. System installation Miraclebox ultra4k."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/miraclebox/ultra4k/rootfs.tar.bz2; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/miraclebox/ultra4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/update/lunix3-4k'):
            os.system(
                'echo "Please wait. System installation Qviart lunix3-4k w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/update/lunix3-4k; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/update/lunix3-4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/update/lunix4k'):
            os.system(
                'echo "Please wait. System installation Qviart Lunix 4K w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/update/lunix4k; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/update/lunix4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/dinobot/u5'):
            os.system(
                'echo "Please wait. System installation dinobot u5 w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/dinobot/u5; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/dinobot/u5/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/dinobot/u51'):
            os.system(
                'echo "Please wait. System installation dinobot u51 w toku..."')
            ...
            os.system(
                'echo "Please wait. System installation dinobot u52 w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/dinobot/u52; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/dinobot/u52/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/dinobot/u53'):
            os.system(
                'echo "Please wait. System installation dinobot u53 w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/dinobot/u53; tar -jxf ' + getNeoLocation() + \
            ...
            os.system(
                'echo "Please wait. System installation dinobot u55 w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/dinobot/uu55; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/dinobot/u55/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
            ...
            if os.path.exists('' + getNeoLocation() + 'ImagesUpload/dinobot'):
                os.chdir('dinobot')
                if os.path.exists(
                        '' + getNeoLocation() + 'ImagesUpload/dinobot/u55'):
                    os.chdir('u55')
                    os.system('echo "Instalacja - ubi_reader w toku..."')
            ...
            os.system(
                'echo "Please wait. System installation dinobot u5pvr w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/dinobot/u5pvr; tar -jxf ' + getNeoLocation() + \
            ...
            os.system(
                'echo "Please wait. System installation dinobot u57 w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/dinobot/u57; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/dinobot/u57/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/uclan/ustym4kpro'):
            os.system(
                'echo "Please wait. System installation ustym4kpro w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/uclan/ustym4kpro; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/uclan/ustym4kpro/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/ustym4kpro'):
            os.system(
                'echo "Please wait. System installation ustym4kpro w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/ustym4kpro; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/ustym4kpro/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/et1x000'):
            os.system(
                'echo "Please wait. System installation GI ET-11000 4K w toku..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/et1x000; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/et1x000/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/e2/update/rootfs.tar.bz2'):
            os.system('echo "Please wait. System installation..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/e2/update; tar -jxf ' + getNeoLocation() + \
            ...
            os.system(
                'echo "Please wait. System installation beyonwiz v2 4K w toku..."')
            ...
            os.system(
                'echo "Please wait. System installation Amiko viper4k 4K w toku..."')
            ...
            os.system(
                'echo "Please wait. System installation trio4k 4K Combo..."')
            ...
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/gigablue/x34k; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/gigablue/x34k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/protek4k'):
            ...
            os.system(
                'echo "Please wait. System installation AB-COM PULSe 4K..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/pulse4k; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/pulse4k/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/pulse4kmini'):
            os.system(
                'echo "Please wait. System installation AB-COM PULSe 4K..."')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/pulse4kmini; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/pulse4kmini/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)
        elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/bre2ze4k'):
        ...
        # Vimastec
            ...
            os.system(
                'echo "Please wait. System installation VIMASTEC VS1500 4K"')
            cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/vs1500; tar -jxf ' + getNeoLocation() + \
                'ImagesUpload/vs1500/rootfs.tar.bz2 -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
            rc = os.system(cmd)

        else:
            os.system(
                'echo "NeoBoot wykrył błąd!!! Prawdopodobnie brak pliku instalacyjnego."')
            ...
            for fn in os.listdir('%sImagesUpload' % getNeoLocation()):
                ...
                if fn.find('.rootfs.tar.xz') != -1:
                    if not os.path.exists('/tmp/xz-gz-tar'):
                        ...
                        os.system(
                            'echo "Installing the file rootfs.tar.xz in progress..."')
                        cmd = 'mv ' + getNeoLocation() + 'ImagesUpload/' + source + '.rootfs.tar.xz  ' + \
                            getNeoLocation() + 'ImagesUpload/rootfs.tar.xz > /dev/null 2>&1'
                        rc = os.system(cmd)
                        cmd1 = 'mv ' + getNeoLocation() + 'ImagesUpload/*rootfs.tar.xz  ' + \
                            getNeoLocation() + 'ImagesUpload/rootfs.tar.xz > /dev/null 2>&1'
                        rc = os.system(cmd1)
                        cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/rootfs.tar.xz; tar -xf ' + getNeoLocation() + \
                            'ImagesUpload/rootfs.tar.xz -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
                        rc = os.system(cmd)
                        rc = os.system(
                            'rm -r ' +
                            getNeoLocation() +
                    ...
                    if not os.path.exists('/tmp/xz-gz-tar'):
                        os.system('touch /tmp/xz-gz-tar')
                        os.system(
                            'echo "Installing the file .tar.xz in progress..."')
                        os.system(
                            'mv ' +
                            getNeoLocation() +
                            'ImagesUpload/' +
                    ...
                    if not os.path.exists('/tmp/xz-gz-tar'):
                        os.system('touch /tmp/xz-gz-tar')
                        os.system(
                            'echo "Installing the file tar.gz in progress..."')
                        os.system(
                            'mv ' +
                            getNeoLocation() +
                            'ImagesUpload/' +
                            ...
                            getNeoLocation() +
                            'ImagesUpload/rootfs.tar.gz')
                        cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/rootfs.tar.gz; /bin/tar -xzvf ' + getNeoLocation() + \
                            'ImagesUpload/rootfs.tar.gz -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
                        rc = os.system(cmd)
                        rc = os.system(
                            'rm -r ' +
                            getNeoLocation() +
                    ...
                    if not os.path.exists('/tmp/xz-gz-tar'):
                        os.system('touch /tmp/xz-gz-tar')
                        os.system(
                            'echo "Installing the file tar in progress..."')
                        os.system(
                            'mv ' +
                            getNeoLocation() +
                            'ImagesUpload/' +
                            ...
                            ' ' +
                            getNeoLocation() +
                            'ImagesUpload/rootfs.tar')
                        cmd = '/bin/tar -xvf ' + getNeoLocation() + 'ImagesUpload/rootfs.tar -C ' + \
                            getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
                        rc = os.system(cmd)
                        rc = os.system(
                            'rm -r ' +
                            getNeoLocation() +
                    ...
                    if not os.path.exists('/tmp/xz-gz-tar'):
                        os.system('touch /tmp/xz-gz-tar')
                        os.system(
                            'echo "Installing the file .tar.bz2 in progress..."')
                        os.system(
                            'mv ' +
                            getNeoLocation() +
                            'ImagesUpload/' +
                    ...
                    os.system(
                        'echo "Please wait. System installation spakowanego w plik .mb w toku..."')
                    os.system(
                        'cp -af ' +
                        getNeoLocation() +
                        'ImagesUpload/' +
                        ...
                        getNeoLocation() +
                        'ImagesUpload/rootfs.tar.gz')
                    cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/*.tar.gz; tar -xzvf ' + getNeoLocation() + \
                        'ImagesUpload/*.tar.gz -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
                    rc = os.system(cmd)

                elif os.path.exists('' + getNeoLocation() + 'ImagesUpload/rootfs.bin') or fn.find('.bin') != -1:
                    ...
                    if not os.path.exists('/tmp/xz-gz-tar'):
                        os.system('touch /tmp/xz-gz-tar')
                        os.system(
                            'echo "Installing the file .gz in progress..."')
                        os.system(
                            'mv ' +
                            getNeoLocation() +
                            'ImagesUpload/' +
                            ...
                            '.gz  ' +
                            getNeoLocation() +
                            'ImagesUpload/rootfs.gz')
                        cmd = 'chmod 777 ' + getNeoLocation() + 'ImagesUpload/rootfs.gz; /bin/tar -xvf ' + getNeoLocation() + \
                            'ImagesUpload/rootfs.gz -C ' + getNeoLocation() + 'ImageBoot/' + target + ' > /dev/null 2>&1'
                        rc = os.system(cmd)
                        rc = os.system(
                            'rm -r ' +
                            getNeoLocation() +
                            '/ImagesUpload/rootfs.gz')
                        cmd = 'rm -rf ' + getNeoLocation() + 'ImagesUpload/' + \
                            sourcefile4 + ' ' ' > /dev/null 2>&1'
                        rc = os.system(cmd)
                        cmd = 'rm -f ' + getNeoLocation() + 'ImagesUpload/*.jpg '  ' > /dev/null 2>&1'
                        rc = os.system(cmd)
    else:
        os.system(f'echo "Image {source} not installed"')
