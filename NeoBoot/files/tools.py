...
class MBRestore(Screen):
    ...
    def dodeleteback(self, answer):
        if answer is True:
            ...
            self.session.open(
                Console, _('NeoBoot: Backup files deleted!'), [
                    cmd, cmd1])
            self.updateInfo()
        else:
            self.close()
...
class MBDeleUpload(Screen):
    ...
    def pedeleup(self, answer):
        if answer is True:
            ...
            self.session.open(
                Console, _('Deleting downloaded image zip files ....'), [
                    cmd, cmd1])
            self.close()
        else:
            self.close()
    ...
    def reinstallneoboot(self, answer):
        if answer is True:
            ...
            cmd6 = 'rm -f ' + getNeoLocation() + 'ImageBoot/initneo.log ' + getNeoLocation() + 'ImageBoot/.imagedistro ' + getNeoLocation() + 'ImageBoot/.neonextboot ' + getNeoLocation() + \
                'ImageBoot/.updateversion ' + getNeoLocation() + 'ImageBoot/.Flash ' + getNeoLocation() + 'ImageBoot/.version ' + getNeoLocation() + 'ImageBoot/NeoInit.log ; sleep 2'
    ...
    def neocheck2(self):
        ...
        if getCPUtype() == 'ARMv7' and getCPUtype() != 'MIPS':
            ...
            if os.system('opkg list-installed | grep python-argparse') != 0:
                os.system(
                    'echo "*    python-argparse not installed"  >>  ' +
                    LinkNeoBoot +
                    '/files/modulecheck')
            ...
            if os.system('opkg list-installed | grep mtd-utils') != 0:
                os.system(
                    'echo "*    mtd-utils not installed"  >>  ' +
                    LinkNeoBoot +
                    '/files/modulecheck')
            if os.system('opkg list-installed | grep mtd-utils-ubifs') != 0:
                os.system(
                    'echo "*    mtd-utils-ubifs not installed"  >>  ' +
                    LinkNeoBoot +
                    '/files/modulecheck')
            else:
                os.system(
...
class ATVcamfeed(Screen):
    ...
    def addcamatv(self):
        if getImageATv() == 'okfeedCAMatv':
            ...
            self.session.open(
                Console, _('NeoBoot: Cams feed add...'), [
                    cmd, cmd1])
...
class IPTVPlayerInstall(Screen):
    ...
    def panel_update(self):
        ...
        if fileExists('/usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer'):
            self.session.open(
                MessageBox,
                _('The plugin IPTVPlayer installed.'),
                MessageBox.TYPE_INFO,
                10)
            self.close()
